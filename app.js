...
var hbs = require("express-handlebars").create({
    ...
    const getPendingSwaps = 'SELECT tbl4.id, tbl4.senderID, tbl4.sendName, tbl4.receiverID, r.username AS reqName, tbl4.bookID, tbl4.title, tbl4.author, tbl4.isbn, tbl4.condition, tbl4.pointsTraded, tbl4.swapDate FROM users r INNER JOIN (SELECT p.id, p.senderID, tbl3.sendName, p.receiverID, tbl3.bookID, tbl3.title, tbl3.author, tbl3.isbn, tbl3.condition, p.pointsTraded, p.swapDate FROM pending_swaps p INNER JOIN (SELECT tbl2.userID, s.username AS sendName, tbl2.bookID, tbl2.title, tbl2.author, tbl2.isbn, tbl2.condition FROM users s INNER JOIN (SELECT u.userID, u.bookID, tbl1.title, tbl1.author, tbl1.isbn, tbl1.condition FROM user_books u INNER JOIN (SELECT * FROM books b) as tbl1 ON u.bookID = tbl1.id WHERE u.userID = ?) as tbl2 ON s.id = tbl2.userID) as tbl3 ON p.senderID = tbl3.userID) as tbl4 ON r.id = tbl4.receiverID';
...
//        mysql.pool.query(, req.params.userID, (err, result) => {
            if (err) {
                console.log('error: ', err);
            } else {
                    contents.swaps = result;
